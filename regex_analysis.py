...
def analyze_dataset_with_regex(data):
    ...
    # Регулярное выражение для поиска цен в фунтах стерлингов
    price_pattern = r'Â£\d+\.\d{2}'
    price_regex = re.compile(price_pattern)
    
    # Регулярное выражение для поиска URL адресов книг
    url_pattern = r'http://books\.toscrape\.com/catalogue/[^/]+_\d+/'
    url_regex = re.compile(url_pattern)
    
    # Регулярное выражение для поиска рейтингов (One, Two, Three, Four, Five)
    rating_pattern = r'\b(One|Two|Three|Four|Five)\b'
    rating_regex = re.compile(rating_pattern)
    
    # Регулярное выражение для поиска информации о доступности
    availability_pattern = r'In stock \(\d+ available\)|Out of stock'
    availability_regex = re.compile(availability_pattern)
    
    # Регулярное выражение для поиска названий издательств (в кавычках)
    publisher_pattern = r'"([^"]+)"\s*--'
    publisher_regex = re.compile(publisher_pattern)
    
    # Регулярное выражение для поиска категорий/жанров книг
    category_pattern = r'\b(?:Fiction|Mystery|Romance|Science|History|Biography|Fantasy|Thriller|Poetry|Art|Music|Travel|Cooking|Business|Self-help|Children|Young Adult|Non-fiction)\b'
    category_regex = re.compile(category_pattern)
    
    # Регулярное выражение для поиска страниц в описаниях
    pages_pattern = r'\b(?:page|pages|p\.|pp\.)\s+(\d+)\b'
    pages_regex = re.compile(pages_pattern)
    
    # Регулярное выражение для поиска годов в описаниях
    year_pattern = r'\b(19|20)\d{2}\b'
    year_regex = re.compile(year_pattern)
    
    # Регулярное выражение для поиска имен авторов (после "by" в описаниях)
    ...
    for i, book in enumerate(data):
        ...
        # Поиск категорий/жанров
        description = book.get('description', '') or ''
        categories = category_regex.findall(description)
        if categories:
            results['patterns_found']['categories'].extend(categories)
        
        # Поиск страниц
        pages = pages_regex.findall(description)
        if pages:
            results['patterns_found']['pages'].extend(pages)
        
        # Поиск годов
        years = year_regex.findall(book.get('description', '') or '')
        if years:
            results['patterns_found']['years'].extend(years)
        
        # Поиск авторов
        authors = author_regex.findall(book.get('description', '') or '')
        if authors:
...
def print_analysis_results(results):
    ...
    print("ПОИСК ЦЕН В ФУНТАХ СТЕРЛИНГОВ")
    ...
    print("ПОИСК ИНФОРМАЦИИ О ДОСТУПНОСТИ")
    ...
    print("ПОИСК ИЗДАТЕЛЬСТВ И ЖУРНАЛОВ")
    ...
    print(f"Найдено: {len(pages)} упоминаний страниц")
    if pages:
        page_numbers = [int(p) for p in pages]
        print(f"Диапазон страниц: от {min(page_numbers)} до {max(page_numbers)}")
